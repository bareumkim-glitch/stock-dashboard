...
// Yahoo Finance API 사용 (API 키 불필요, 무료)
...
const StockDashboard = () => {
  ...
  const [marketData, setMarketData] = useState({
    indices: [
      { name: 'KOSPI', symbol: 'KOSPI', price: 0, change: 0, changePercent: 0, loading: true },
      ...
      { name: '원/달러', symbol: 'USDKRW', price: 0, change: 0, changePercent: 0, loading: true },
    ]
  });
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, (currentUser) => {
      setUser(currentUser);
      if (currentUser) {
        fetchAllData(); // 로그인하면 바로 데이터 가져오기
      }
    });
  ...
  // TIGER 200 ETF 데이터
  const fetchTiger200 = async () => {
    try {
      const response = await fetch(
        `https://query1.finance.yahoo.com/v8/finance/chart/102110.KS?interval=1d&range=1d`
      );
      ...
      console.error('TIGER 200 데이터 오류:', error);
      updateMarketData('102110.KS', { loading: false });
    }
  };
  ...
  const fetchKOSPI = async () => {
    try {
      ...
      console.error('KOSPI 데이터 오류:', error);
      updateMarketData('KOSPI', { loading: false });
    }
  };
  ...
  const fetchExchangeRate = async () => {
    try {
      ...
      updateMarketData('USDKRW', { loading: false });
    }
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 p-4">
      <div className="max-w-[1024px] mx-auto">
        ...
        <div className="mt-6 text-center text-slate-500 text-xs">
          ...
          <p>※ Yahoo Finance API를 통해 실시간 데이터 제공</p>
          <p className="mt-1">※ 데이터는 약간의 지연이 있을 수 있습니다</p>
        </div>
      </div>
    </div>
  );
